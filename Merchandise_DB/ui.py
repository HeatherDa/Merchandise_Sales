        ...
        if (len(da[0]) == 10) & ((len(da[1]) == 5)|(len(da[1]) == 8)):
            ...
            for char in da[1]:
                if char in '0123456789':
                    ...
                    colon = colon + 1
            if ((count+dash) == len(da[0])) & ((count2+colon) == len(da[1])) & (count == 8) & (dash == 2) & \
                    (((count2 == 4) & (colon == 1)) | (count2 == 6) & (colon == 2)):
                                                        #count2 could be HH:MM, or HH:MM:SS
...
def profits_header():
    ...
    show_message("\033[1m" + "\033[4m"
                 + add_spaces("Item ID:", 'item_ID')
                 + add_spaces("Item Type:", 'item_Type')
                 + add_spaces("Item Description", 'item_Description')
                 + add_spaces("Profit: ", 'sales_Profit')
                 + "\033[0m")
...
def profit_result_format(record):
    k = record.keys()
    a = ""
    for col in k:
        if col in ('SUM(event_sales.sales_Profit)', 'SUM(sales_Profit)'):
            a = a + add_spaces(str(record[col]), 'sales_Profit')
        elif col != 'item_Taxable':
            a = a + add_spaces(str(record[col]), str(col))
    show_message(a)
